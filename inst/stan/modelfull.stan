...
transformed parameters {
 ...
 lambda[1,1] = omega[1,1] + A[1,1]*mean(Y[,1]) + B[1,1]*med_y1 + B[1,2]*med_y2;
 lambda[1,2] = omega[2,1] + A[2,2]*mean(Y[,2]) + B[2,2]*med_y2 + B[2,1]*med_y1;

  for(t in 2:n){
    lambda[t,] = to_row_vector( to_vector(omega') + A*(lambda[t-1,]') + B*(Y[t-1, ]'));
  }
}
model {
