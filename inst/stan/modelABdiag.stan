...
parameters {
    ...
    vector<lower =0, upper = 1-max(B)>[2] A;
...
transformed parameters {
  ...
  matrix[2,2] I;
  matrix<lower = 0>[2,2] A_mat;
  matrix<lower = 0>[2,2] B_mat;

  I = diag_matrix(rep_vector(1.0, 2));
  A_mat = diag_matrix(A);
  B_mat = diag_matrix(B);

  lambda[1,1] = omega[1,1] + A_mat[1,1]*mean(Y[,1]) + B_mat[1,1]*med_y1 + B_mat[1,2]*med_y2;
  lambda[1,2] = omega[2,1] + A_mat[2,2]*mean(Y[,2]) + B_mat[2,2]*med_y2 + B_mat[2,1]*med_y1;

  for(t in 2:n){
    lambda[t,] = to_row_vector( to_vector(omega') + A_mat*(lambda[t-1,]') + B_mat*(Y[t-1, ]'));
  }
}
model {
